...
class Railway2ModelTickets extends JModelList
{
    ...
    public function getListQuery()
    {
        ...
        $query
            ->select('`desc`.`id` as `id`, `s`.`id` as `sid`, `s`.`name`, `name`.`popularName`, `desc`.`timemask`, `desc`.`turnstiles`, `desc`.`time_1`, `desc`.`time_2`, `code`.`esr`, `code`.`express`, `t`.`title` as `type`, `reg`.`region` as `region`, `rw`.`road`, `rw`.`division`, `dir`.`title` as `direction`, `desc`.`time_check`')
